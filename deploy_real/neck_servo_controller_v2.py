...
class NeckServoController:
    ...
    def read_homing_offset(self, dxl_id):
        ...
        if result != COMM_SUCCESS:
            ...
            return None
        #! NOTE(AHMED): converting the value to signed int, packet handle is dumb and doesnot understand signedness
    ...
    def write_homing_offset(self, dxl_id, offset):
        ...
        # NOTE(AHMED) signed int to unsigned for transmission
        if offset < 0:
            offset = offset + 0x100000000
    ...
    def calibrate(self):
        ...
        - Yaw motor: current position becomes 180 deg
        - Pitch motor: current position becomes 145 deg
        
        This method is called after manually positioning the motors to their desired center/home position.
        Calibration works as follows:
        - disable torque, we cant edit eeprom area with torque enabled
        - read current homing offset
        - set the offset to 0
        - get current position (f we cant get current position we reset the homing offset)
        - compute new offset so the current position becomes the center
        ...
        for idx, dxl_id in enumerate(self.DXL_IDS):
            ...
            # Note(AHMED): Torque must be disabled to write to homing offset (EEPROM area).
            ...
            # Present Position = Raw Position + Homing Offset
            ...
            if verify_position is not None:
                ...
                if abs(verify_position - target_center_ticks) > 5:
                    print(f"  [yellow]Warning: Position is {verify_deg:.1f} deg instead of expected {target_center_deg} deg[/]")
    ...
    def setup_dynamixel(self):
        ...
        for idx, dxl_id in enumerate(self.DXL_IDS):
            ...
            # NOTE: Torque must be disabled to change operating mode and limits
            if not self.set_torque(dxl_id, False):
    ...
    def dxl_to_rad_position(self, dxl_pos, motor_index):
        ...
        if motor_index == 0:  # Yaw motor
            # Positive = rotate right (relative to center 180 deg)
            deg = motor_deg - center_deg
        elif motor_index == 1:  # Pitch motor
            # Inverted: positive = look up (relative to center 145 deg)
    ...
    def rad_to_dxl_position(self, rad, motor_index):
        ...
        Args:
            rad: Angle in radians (SMPL derived neck angle, 0 = straight/center)
            motor_index: 0 for yaw, 1 for pitch
            
        Returns:
            Dynamixel position units, clamped to motor's safe range
        """
        deg = rad * (180.0 / np.pi)
        center_deg = self.MOTOR_CENTER_DEG[motor_index]
        ...
        if motor_index == 0:  # Yaw motor
            # Positive deg = rotate right
            motor_deg = center_deg + deg
        elif motor_index == 1:  # Pitch motor
            # Inverted: positive SMPL pitch (look up) -> decrease motor position
            motor_deg = center_deg - deg
        else:
        ...
        dxl_pos = self.deg_to_ticks(motor_deg)
    ...
    def get_neck_from_redis(self):
        ...
        try:
            ...
            if data is not None:
                ...
                if len(neck_data) >= 2:
                    # Use yaw from Redis, ignore pitch if disabled (use 0.0)
                    yaw = neck_data[0]
                    pitch = neck_data[1] if self.ENABLE_PITCH_MOTOR else 0.0
                    self.last_neck_data = [yaw, pitch]
...
def main():
    ...
    parser.add_argument("--calibrate", action="store_true",
                        help="Calibrate motors on startup (Yaw->180 deg, Pitch->145 deg)")
