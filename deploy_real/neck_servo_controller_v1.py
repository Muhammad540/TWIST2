...
class NeckServoController:
    ...
    YAW_MOTOR_RANGE_DEG = (90, 270.0)     # (min, max)
    ...
    def setup_dynamixel(self):
        ...
        for idx, dxl_id in enumerate(self.DXL_IDS):
            ...
            if idx == 1 and not self.ENABLE_PITCH_MOTOR:
                print(f"[yellow]Skipping {motor_name} motor (ID {dxl_id}) - pitch control disabled[/]")
                continue
    ...
    def set_position(self, neck_data):
        ...
        for i, dxl_id in enumerate(self.DXL_IDS):
            if i == 1 and not self.ENABLE_PITCH_MOTOR:
                continue
